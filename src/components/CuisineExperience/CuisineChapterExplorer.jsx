...
export default function CuisineChapterExplorer({ chapterKeys }) {
  ...
  return (
    <section className="py-24 bg-[#0a0908] border-t border-[#CBAA6A]/10">
          ...
          {chapterKeys.map((chapterSlug, idx) => {
            ...
            return (
              <Link
                ...
                className="group relative min-w-[85vw] md:min-w-0 h-[400px] flex-shrink-0 snap-center rounded-2xl overflow-hidden border border-[#CBAA6A]/10 block cursor-pointer bg-[#0a0908] hover:border-primary-gold/40 transition-colors duration-500"
              >
                ...
                <div className="absolute inset-0 p-8 flex flex-col justify-end">
                  <span className="font-sans text-[10px] tracking-[0.3em] uppercase text-primary-gold mb-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform translate-y-2 group-hover:translate-y-0">
                    Open Chapter
                  </span>
                  <h3 className="font-serif text-3xl text-primary-cream mb-2 group-hover:text-primary-gold transition-colors duration-300">
                    {chapter.title}
                  </h3>
                  <p className="font-sans text-sm text-primary-cream/60 line-clamp-2">
                    {chapter.description}
                  </p>
                </div>
              </Link>
