...
export default function InteractiveExplorer({ dishes }) {
  ...
  return (
    <section id="explorer" className="py-24 bg-surface-base relative">
      <div className="max-w-7xl mx-auto px-6">
        
        {/* Header & Sticky Controls */}
        <div className="sticky top-20 z-30 bg-surface-base/95 backdrop-blur-md py-6 border-b border-border-subtle mb-12 flex flex-col items-start gap-6">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between w-full gap-6">
            <div>
              <h2 className="text-3xl font-serif text-content-primary">Complete Menu</h2>
              <p className="text-content-secondary text-sm mt-1">Explore the full range of delicacies</p>
            </div>
        ...
        {filteredDishes.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredDishes.map(dish => (
              <div key={dish.id} className="group p-6 bg-surface-card border border-border-subtle rounded-xl hover:border-accent-gold transition-colors duration-300">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-xl font-serif text-content-primary group-hover:text-accent-gold transition-colors">{dish.name}</h3>
          ...
          <div className="text-center py-24 border border-dashed border-border-subtle rounded-2xl">
            <p className="text-content-secondary text-lg">No dishes found for the selected criteria in this category.</p>
